...
export default function Perfil() {
  ...
  const fetchAlertas = async (paginaAtual = 0) => {
    try {
      setLoading(true);
      const res = await fetch(`http://15.228.204.198:8080/alertas/meus-alertas?page=${paginaAtual}&size=10`, {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        const userRes = await fetch("http://15.228.204.198:8080/usuarios/me", {
          headers: { Authorization: `Bearer ${token}` },
        });
  ...
  const deletarContato = async (id: number) => {
    try {
      const response = await fetch(
        `http://15.228.204.198:8080/usuarios/me/contatos/${id}`, {
        method: "DELETE",
        headers: { Authorization: `Bearer ${token}` },
      }
      );
  ...
  const salvarContato = async () => {
    ...
    try {
      const response = await fetch("http://15.228.204.198:8080/usuarios/me/contatos", {
        method: "POST",
        headers: {
